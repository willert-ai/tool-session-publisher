...
- **x-comms-engine v1 — MERGED to `main` (PR #1, 11 commits). Register chain in flight on `feat/x-comms-register`.** A launchd user agent (`ai.fero.x-comms`, daily 07:30) runs `skill/engine/run.sh` with zero Claude sessions open: it expires the queue, mines the session history for seeds, drafts finished bodies through a headless `claude -p`, and files them into a queue **outside this repo**. Sunday is a deep tick (14-day window, ≤3 drafted as one arc); every other day is ambient (3 days, ≤2 singles). Review with `python3 skill/helpers/queue.py review`. Nothing publishes itself. **D1 is proven in production** — unattended fires 2026-08-27 and 2026-08-28, nobody present. **The mechanism works; the output is not yet postable.** The operator has rejected 11 of 11 drafts. Two root causes, found in that order and both fixed: seed depth (the miner read a ~1.2 KB index row while the ~23 KB session document sat beside it — now resolves the document and builds `text` from its narrative sections, `seed_ref` still the verbatim row); and **register** — `persona.local.md` §4 prescribed a ≤280-char staccato voice with a mandatory fragment-length takeaway, derived from the operator's 5 earliest posts (7–39 impressions, zero engagement). The engine obeyed it exactly. Rebuilt 2026-08-28 from a 33-draft operator-verdicted calibration: **400–650 characters, three or four short paragraphs, framing question allowed as the opening line only, landing on a plain-language takeaway.** The five laws live in `skill/engine/task-single.md`. **Merge of the register chain is gated on the operator wanting to publish something from a real queue** — their judgement, not a passing test. Authoritative design: the local, gitignored `planning/SPEC-x-comms-engine.md` rev 2, amended at D9 and superseded on length/register by §4 of the persona. Per-commit history lives in `planning/progress.md` + `planning/findings.md` and the commit messages — **not here**.
